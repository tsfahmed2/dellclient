...
else
{
    ...
    function Get-Architecture {
        ...
        if ((Get-CimInstance win32_bios).Manufacturer -match 'Dell')  {
            if (($EXE -eq $null) -or ((Get-Item $EXE).VersionInfo.FileVersion -ne '3.1.0.64')) {
                ...
                Invoke-WebRequest -Uri "https://dl.dell.com/FOLDER05944435M/1/Dell-Command-Update_0NJ7C_WIN_3.1.0_A00.EXE" -OutFile "$env:temp\DCU.exe" -UseBasicParsing -Verbose
            ...
            $a = "/configure -updateSeverity=critical,recommended,optional -scheduleWeekly=Mon,11:45 -updatetype=driver,bios,firmware,others -updateDeviceCategory=network,storage,audio,video,input,chipset -scheduleAction=DownloadInstallAndNotify -autoSuspendBitLocker=enable -outputLog=$setlog"
